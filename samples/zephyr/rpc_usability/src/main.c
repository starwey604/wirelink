...
static void done(void *context, const wl_rpc_completion_t *result, const response_value_t *output) {
  ...
  CHECK(rpc_validation_endpoint_close(&client) == WL_ERR_REENTRANT);
  CHECK(rpc_validation_endpoint_step(&client) == WL_ERR_REENTRANT);
  if (closing) {
  ...
  if (chained != 0) {
    request_value_t value = request();
    --chained;
    CHECK(rpc_validation_endpoint_execute_async(&client, &value, 100, done, &completions, &handle) == WL_OK);
  }
...
static void initialize(void) {
  ...
  CHECK(rpc_validation_endpoint_init_config(&server, &config) == WL_OK);
  CHECK(wl_loopback_connect(&cable, rpc_validation_endpoint_handle(&client), rpc_validation_endpoint_handle(&server)) == WL_OK);
}
static void step(void) {
  unsigned before = clock_reads;
  CHECK(rpc_validation_endpoint_step(&client) == WL_OK);
  CHECK(rpc_validation_endpoint_step(&server) == WL_OK);
...
static void close_pair(void) {
  unsigned before = clock_reads;
  closing = true;
  CHECK(rpc_validation_endpoint_close(&client) == WL_OK);
  CHECK(rpc_validation_endpoint_close(&server) == WL_OK);
  CHECK(rpc_validation_endpoint_close(&client) == WL_OK);
  closing = false;
...
int main(void) {
  ...
  for (unsigned i = 0; i < 100; ++i) {
    ...
    CHECK(rpc_validation_endpoint_execute_async(&client, &value, 100, done, &completions, NULL) == WL_OK);
    wait_for(target);
  }
  CHECK(handlers == 101);
  target = completions + RPC_VALIDATION_ENDPOINT_RPC_CAPACITY;
  for (unsigned i = 0; i < RPC_VALIDATION_ENDPOINT_RPC_CAPACITY; ++i)
    CHECK(rpc_validation_endpoint_execute_async(&client, &value, 100, done, &completions, NULL) == WL_OK);
  CHECK(rpc_validation_endpoint_execute_async(&client, &value, 100, done, &completions, NULL) == WL_ERR_BUSY);
  wait_for(target);
  ...
  expected = WL_RPC_SUCCESS;
  {
    ...
    target = completions + 1;
    CHECK(rpc_validation_endpoint_download_async(&client, &empty, 100, download_done, NULL, NULL) == WL_OK);
    wait_for(target);
  }
  /* Rejected admission never has a completion, nor overwrites cancellation authority. */
  value = request();
  before = completions;
  CHECK(rpc_validation_endpoint_execute_async(&client, &value, 0, done, &completions, &handle) == WL_ERR_INVALID_ARG);
  ...
  for (unsigned i = 0; i < RPC_VALIDATION_ENDPOINT_RPC_CAPACITY; ++i)
    CHECK(rpc_validation_endpoint_execute_async(&client, &value, 10, done, &completions, NULL) == WL_OK);
  now += 11U;
  wait_for(target);
  ...
  before = clock_reads;
  target = completions + 1;
  /* Functional uptime check, not a 100-ms host/QEMU scheduling benchmark. */
  CHECK(rpc_validation_endpoint_execute_async(&client, &value, 2000, done, &completions, NULL) == WL_OK);
