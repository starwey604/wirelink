...
the same codegen contract because public interfaces, layouts and wire formats
do not change.
...
machine/workload mismatches. Compiler/flags need separate provenance
verification. Correctness smoke is not a timing gate; no fixed threshold is
imposed on shared CI.
...
benchmark process at a time, with no CTest, Twister, compiler, sanitizer or
other benchmark running alongside it. On hybrid/mixed-frequency hosts, pin
**both** frozen executables to the same suitable CPU, for example `taskset -c 1`
on Linux after checking `lscpu -e=CPU,CORE,MAXMHZ`. Record the affinity; do not
compare a pinned run against an unpinned run or mix the two datasets. Frequency
...
## ESP32-S3 and instruction cache

Build `benchmarks/zephyr/codec_plan` from an initialized Zephyr workspace,
selecting the frozen `CODEC_PLAN_CODEC_DIR`. Both sides must use the same
...
Each group has five samples: **225 groups / 1,125 samples per boot**. Timer
reads bracket each operation, with compiler memory barriers to prevent LTO
hoisting work across timing. IRQ masking covers only the test operation; cache
invalidation, validation, logging and sleeping are outside measured cycles. The
...
Free FLASH in this app does not establish product headroom.
