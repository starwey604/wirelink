...
def capture(lto=1):
    ...
    rows += [f"codec_plan_v1,m={m},k={k},p={p},s={s},r={r},n={36 if m == 2 else 32},cycles=3200"
             for m, k, p, s in sorted(compare.FIRMWARE) for r in range(5)]
    return "\n".join(rows + ["codec_plan_end_v1,groups=225,samples=1125,result=pass"])


class Tests(unittest.TestCase):
    def test_complete(self):
        parsed = compare.firmware(capture())
    ...
    def test_lto_mismatch(self):
        with self.assertRaises(ValueError):
            list(compare.compare(compare.firmware(capture(0)), compare.firmware(capture(1))))
