...
[English](EXECUTOR.md)。本基准单独量化线程之间的提交、唤醒、结果通知与 LATEST 替换。
...
初始化在测量前；本基准不含 socket、USB、CAN 或电机业务。`proxy` 不是 RPC 网络基准。
Windows 的进程 CPU 读数使用 `GetProcessTimes`，不使用当地返回墙时的 `std::clock`；
正式测量请在空闲主机上进行，跨平台源码不等同于 Windows/macOS 性能验收。
...
计数口径及独立正确性用例见 [owner-pass harness](../owner_pass/README-cn.md)。
