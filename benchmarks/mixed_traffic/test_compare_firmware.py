...
from compare_firmware import CPU_KEYS, RETRY_KEYS, parse, summarize
from test_compare import fixture
import json


def capture():
    lines = ["mixed_fw_begin_v1,hz=550000000,lto=1,coexist=1,endpoint_bytes=3888,barriers=1"]
    ...
    lines += ["mixed_fw_end_v1,result=pass,traffic_rows=42,cpu_groups=21,retry_samples=360"]
    return "\n".join(lines)


class FirmwareCaptureTests(unittest.TestCase):
