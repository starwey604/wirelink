...
`previous/control.wl` and `current/control.wl` are a compatible schema pair. The
checked-in C artifacts are generated by a matching, independently installed WLC;
the core build and fixture-based tests do not require Rust or WLC.
...
schema/profile identities, codegen contract, and artifact digests. The profiled
current revision also contains the application runtime pair
(`control_runtime.h/.c`). It claims caller-owned LATEST or FIFO storage, decodes
directly into the claimed slot, publishes only complete values, and releases each
valid RX event. The current artifact drives the Astrial typed serial test, the
application-runtime integration test, and the bulk-transfer integration test.
Both codec revisions are compiled independently by the Zephyr unit fixture to
exercise forward/backward payload compatibility. Canonical fingerprints use a
generated private sink instead of a canonical byte buffer, and owned conversion
reuses validated decode results.
