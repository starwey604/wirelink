...
Generated C++20 / Python 3.10+ SDK for `device`, version `0.8.0`.
The source API is binding revision 2. It supports managed,
...
```sh
python -m pip install /path/to/wirelink-device-sdk-VERSION-PYTHON-ABI-PLATFORM.whl
```
...
Build/install the matching Wirelink 0.8.0 checkout with
...
```cmake
find_package(DeviceSdk CONFIG REQUIRED)
target_link_libraries(my_app PRIVATE DeviceSdk::client)
```

Include `<device/client.hpp>` and use `device::Client::connect(UdpOptions)`.
...
This SDK does not provide subscriptions, Serial/USB, Bulk, dynamic
