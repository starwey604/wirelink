...
| 12 | 8 | 原客户端 session，非零无符号大端整数；响应原样回送 |
...
session 为零也属于畸形元数据；可靠请求身份须与链路头一致。不匹配本地身份的响应
只报告 `WL_RPC_ERR_SESSION_MISMATCH` 诊断，不改变其他调用。v1/v2 不自动互通。
...
预留的缓存区。链路与响应容量自动包含 20 字节前缀；请求指纹只覆盖业务规范编码，在本地计算，
不作为字段发送。
...
托管 v2 使用原客户端 session 与编号共同匹配响应；端点重建自动生成新身份，旧实例的
同号响应不会完成新调用。不可靠请求也携带身份并划分缓存。身份不是认证或有序版本，
不保证任意旧请求不会再次执行。显式映射模式没有新增该字段，仍须由集成保证旧响应
隔离。来源与耗尽规则见[自动会话](session-cn.md)。
