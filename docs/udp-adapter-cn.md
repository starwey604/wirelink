...
端点初始化时接收时钟。可靠提交取一次时间，同时用于链路和 RPC；step 在适配器完成
通知前取本轮时间，立即回复复用它。发送前不再需要预先 step。
step 外查询 hint 只取时间，不推进状态。详见[时钟契约与迁移](endpoint-clock-cn.md)。
UDP 就绪只负责唤醒 owner，不替代时钟和截止时间策略。
