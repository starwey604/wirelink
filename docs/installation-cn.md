...
本轮期望分别输出 `wlc 0.7.0-rc.1` 和 `32`。请检查将传给 CMake 的那个可执行文件。
...
macOS x86-64/arm64 发布包，固定下载 `v0.7.0-rc.1`，校验硬编码 SHA256 后再检查
