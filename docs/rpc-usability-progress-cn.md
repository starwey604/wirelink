...
## M5：产品 dev 与可编译教程（本轮软件与 H3 验收通过）
...
H3 首轮：14:44 首次换烧失败，用户重新拔插/松开后，15:09 普通 USB HIL 成功烧录并校验。
...
15:43 用户松开 RESET 后正常连接，保持同一 Commander + RTT Telnet 会话完成后续换烧。
有效板端数据发现 HIL 没有复用正式 Willow 已有的遥测调度器，直接连续发送导致诊断
始终被 busy 挡住；这不是正式 Willow 调度器的新故障。Ragtime `4d3efb2` 让 HIL
复用该调度器，补充计数和 `--require-diagnostics` 检查，不改变 Wirelink 核心/ABI/帧格式。
旧镜像在新增门槛下按预期失败，修正版每个 20 秒正式窗口收到 40 条诊断。
新增模拟断言随 native_sim/64 与 M3 QEMU 共 40/40 用例通过，Linux/MSVC 构建通过。

修正版配对测量与无调试进程测量各 3 × 10000 次回显，六个窗口全过、零错配/缺帧。
无探针组 p99 2.088–2.092 ms、Host CPU 7.58–8.91%；配对组 H7 owner 平均
21.064–21.101 us/pass，周期折算窗口占比 3.116–3.138%，含抢占和收尾，非纯线程 CPU。
协议溢出/分发/完成错误、owner deadline miss 均为零。真实 USB disable/reenable 后
回调各一次，RX 恢复，产品 RPC、1000 次回显及诊断恢复通过；已烧回普通修正版 HIL。
随后无探针下对抗性重连 2/2、产品生命周期 100/100 及上述长测全部通过。

本轮 M5/H3 固定快照验收通过，完整配对、边界、哈希和开放项见
[H3 调度修正续验](rpc-h3-scheduler-validation-cn.md)。首轮单次打开失败及早期超标 gap
未获得完整根因证据，不因复测通过而宣称全部已修复；留作后续长稳观测。
没有 main 合并、tag 或发布，Ragtime 新增量仍经 bundle 同步，未推送既有未推提交。
