...
日期：2026-09-07。本页保留 **15:25 前的首轮记录**：软件门槛通过，物理 USB 部分通过。
之后恢复调试连接、发现并修正 HIL 遥测调度缺口、完成板端配对及 USB 重枚举，见
[H3 调度修正续验](rpc-h3-scheduler-validation-cn.md)。下文的当前状态/待办均指首轮结束时。
