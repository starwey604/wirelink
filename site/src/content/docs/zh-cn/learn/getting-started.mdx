...
链路和接收端会分别作出决定。UDP 链路上可能丢失采样；采样到达以后，`latest` 存储位会
用新值替换旧值：

```mermaid
flowchart LR
  accTitle: latest 值的数据路径
  accDescr: 采样先经过可能丢包的 UDP 链路，订阅端再为显示端保留最近收到的一个值。
  Publisher["发布端<br/>采样 1 到 5"]
  Link["UDP 链路<br/>可能丢失采样"]
  Endpoint["订阅端 endpoint"]
  Latest["latest 存储位<br/>只保留一个值"]
  Display["显示端读取<br/>当前值"]
...
当控制端需要取得某次操作对应的结果，而不是读取当前保留值时，继续学习
[向设备请求计算结果](../request-a-result/)。
