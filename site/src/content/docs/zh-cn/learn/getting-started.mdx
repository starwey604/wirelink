...
UDP 不保证送达，因此中间采样不一定全部出现。订阅端收到第 5 个采样后退出；十秒内没有收到则报告超时。

链路和接收端会分别作出决定。UDP 链路上可能丢失采样；采样到达以后，`latest` 存储位会用新值替换旧值：
...
这份 **schema（消息定义）** 给两个 endpoint 提供相同的字段名、类型和稳定编号。endpoint 编码这些字段，而不是直接发送 C 结构体的内存布局。
...
latest 值保留描述数据**收到以后**如何保存。unreliable 投递描述数据在**链路上**如何发送。温度会持续刷新，通常可以容忍某个中间读数丢失，所以本例把两者组合使用。
