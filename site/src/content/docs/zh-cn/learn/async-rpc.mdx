...
本地提交与最终完成之间，由应用主动推进。一次已接受的调用会经过几个状态，最终以成功、业务拒绝或超时之一结束：

```mermaid
stateDiagram-v2
  accTitle: 一次异步 RPC 的生命周期
  accDescr: add_async 返回 WL_OK 只表示本地已接受；之后由 owner 的 step() 推进，最终以成功、拒绝或超时等终态进入完成回调。
  state "已提交" as Submitted
  state "在途" as InFlight
  state "成功" as Succeeded
  state "业务拒绝" as Rejected
  state "超时" as TimedOut
  [*] --> Submitted: add_async() 返回 WL_OK
  Submitted --> InFlight: owner 调用 step()
  InFlight --> Succeeded: step() 收到成功响应
  InFlight --> Rejected: step() 收到业务拒绝
  InFlight --> TimedOut: 1500 ms 预算到期
  Succeeded --> [*]: 回调交付 result 与 response
  Rejected --> [*]: 回调交付 result.rejection
  TimedOut --> [*]: 回调交付 result.status
```
...
异步提交之后，调用仍然需要存活的 endpoint、持续推进，以及在完成或有序关闭前始终有效的应用状态。
...
这个 `addition_t` 变量一直存在于 `main()` 中，直到 endpoint 关闭以后，因此回调上下文可以安全地指向它。
...
`calculator_endpoint_step()` 执行一次有界推进：服务传输、处理收到的数据、推进 deadline，并可能调用完成回调。随后 `example_udp_wait()` 会等待 UDP 就绪或短暂超时；等待只报告就绪，真正的推进仍由 `step()` 完成。
